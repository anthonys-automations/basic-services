...
- **Desktop defaults for a container.** XFCE's `WebBrowser` helper is `debian-sensible-browser` by default, which finds no browser here and fails the panel launcher with "Input/output error"; it points at the stock `firefox` helper instead, which resolves to the bundled wrapper. Compositing is disabled because Xvnc already provides one, `chansrv`'s FUSE drive redirection because `/dev/fuse` is not available, and `NO_AT_BRIDGE` is set because no accessibility bus runs. `/tmp/.X11-unix` and `/tmp/.ICE-unix` are created at build time, since a non-root X server refuses to create them. What is left in the log - missing `pm-is-supported`, `xkbcomp` keysym warnings, panel plugins the image does not install, PulseAudio finding no home directory of its own - is cosmetic.
...
- **Read-only root filesystem.** The Kubernetes example mounts an `emptyDir` over `/home/user`, `/tmp`, `/run/sandbox`, `/run/xrdp` and `/dev/shm`. Each one hides what the image prepared there, so the entrypoint recreates `/run/xrdp/sockdir`, `/tmp/.X11-unix` and `/tmp/.ICE-unix`, and copies `/etc/skel` back into the home directory. It also costs `sshd` its `StrictModes` check: a volume root is world-writable and owned by the kubelet, and the check stats every directory above `authorized_keys` - up to `/`, or up to and including the home directory - so putting the file in a private subdirectory does not help. The copy itself is still written mode 600 as `user`, and no other account in the container can be reached without `setuid`, which `no_new_privs` already blocks.
- **Replaced PAM stacks.** Both distribution stacks authenticate or account through `pam_unix`, which needs `/etc/shadow`. A non-root process cannot read it, so `pam_unix` defers to the setuid `unix_chkpwd` helper - and that helper gains nothing under `no_new_privs`, which is what `allowPrivilegeEscalation: false` sets, so SSH logins were refused with "Access denied for user user by PAM account configuration" despite a valid key. `/etc/pam.d/sshd` is reduced to `pam_permit` plus the session hook, which is safe because `sshd` is public-key only; `/etc/pam.d/xrdp-sesman` authenticates against `/run/sandbox/xrdp.passwd`, where `XRDP_PASSWORD` is hashed at startup. Test images with `--security-opt no-new-privileges` or this class of failure only appears in the cluster.
